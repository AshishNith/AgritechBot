...
export function OtpScreen({ navigation, route }: Props) {
  ...
  const pendingOtp = useAppStore((state) => state.pendingOtp);
  ...
  const verifyMutation = useMutation({
    mutationFn: () => apiService.verifyOtp(phone, otp),
    onSuccess: (data) => {
      setError(null);
      setToken(data.token);
      setUser(data.user);
    ...
    onError: (error: any) => {
      ...
      const message = error?.response?.data?.error || t(language, 'invalidOrExpiredOtp');
      setError(message);
    },
  });

  const resendMutation = useMutation({
    mutationFn: () => apiService.sendOtp(phone),
    onSuccess: (data) => {
      setError(null);
      setOtp('');
    ...
    onError: (error: any) => {
      const message = error?.response?.data?.error || t(language, 'failedToResendOtp');
      setError(message);
    },
  });
